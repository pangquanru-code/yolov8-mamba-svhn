...
class VSSBlock(nn.Module):
    """
    Visual State Space Block (Mamba 核心封装)
    作用：将 2D 图像特征图展平为 1D 序列，送入 Mamba 进行全局序列建模，再还原为 2D。
    """
    def __init__(self, c):
        super().__init__()
...
class C2f_Mamba(nn.Module):
    """
    魔改版 C2f: 将内部的 Bottleneck 替换为 Mamba VSSBlock
    保持了 C2f 的跨层特征拼接逻辑(CSP结构)，确保梯度流的稳定性
    """
